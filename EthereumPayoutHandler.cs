    ...
    IPayoutHandler
{
                    ...
                    if(latestBlockHeight - block.BlockHeight >= EthereumConstants.MinConfimations)
                    {
                        ...
                        if(totalDuplicateBlock > 1 && totalDuplicateBlockAfter > 0)
                        {
                            ...
                            if(string.Equals(blockMiner, poolConfig.Address, StringComparison.OrdinalIgnoreCase))
                            {
                                ...
                                if(extraPoolConfig?.ChainTypeOverride == "Ethereum" || extraPoolConfig?.ChainTypeOverride == "Main" || extraPoolConfig?.ChainTypeOverride == "MainPow" || extraPoolConfig?.ChainTypeOverride == "Ubiq" || extraPoolConfig?.ChainTypeOverride == "EtherOne" || extraPoolConfig?.ChainTypeOverride == "Metachain" || extraPoolConfig?.ChainTypeOverride == "Hypra")
    ...
    public async Task PayoutAsync(IMiningPool pool, Balance[] balances, CancellationToken ct)
    {
        ...
        if((networkType == EthereumNetworkType.Main || extraPoolConfig?.ChainTypeOverride == "Classic" || extraPoolConfig?.ChainTypeOverride == "Mordor" || networkType == EthereumNetworkType.MainPow || extraPoolConfig?.ChainTypeOverride == "Ubiq" || extraPoolConfig?.ChainTypeOverride == "EtherOne" || extraPoolConfig?.ChainTypeOverride == "Metachain" || extraPoolConfig?.ChainTypeOverride == "OctaSpace" || extraPoolConfig?.ChainTypeOverride == "OctaSpaceTestnet" || extraPoolConfig?.ChainTypeOverride == "Hypra") &&
    ...
    internal static decimal GetBaseBlockReward(GethChainType chainType, ulong height)
    {
        switch(chainType)
        {
            ...
            case GethChainType.Metachain:
               return MetachainConstants.BaseRewardInitial;
    ...
    private async Task<string> PayoutAsync(Balance balance, CancellationToken ct)
    {
        ...
        if(extraPoolConfig?.ChainTypeOverride == "Ethereum" || extraPoolConfig?.ChainTypeOverride == "Main" || (extraPoolConfig?.ChainTypeOverride == "Ubiq") || extraPoolConfig?.ChainTypeOverride == "Metachain" || extraPoolConfig?.ChainTypeOverride == "EtherOne" )
        {
        ...
        if(extraPoolConfig?.ChainTypeOverride == "Metachain")
        {
            var requestMetachain = new SendTransactionRequestMetachain
            {
                From = poolConfig.Address,
                To = balance.Address,
                Value = amount.ToString("x").TrimStart('0'),
            ...
            response = await rpcClient.ExecuteAsync<string>(logger, EC.SendTx, ct, new[] { requestMetachain });
        }  
        else {
